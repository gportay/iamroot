...
and it's options (i.e. `--preload`, `--library-path`, `--argv0`...).
...
The C library [glibc] leaks symbols in the dynamically linked binaries and
shared libraries.
...
binaries, then [iamroot(7)] rewrites the whole command line to overcome all
these limitations mention above.
...
against former version of glibc with missing new symbols.
...
[fakechroot(1)], [fakeroot(1)], [pseudo(1)], [binfmt_misc], [qemu]
