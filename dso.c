...
static ssize_t __fldso_access(int fd,
			      int mode,
			      const char *exec_rpath,
			      const char *exec_runpath,
			      const char *rpath,
...

{
	...
	siz = fpath(fd, path, sizeof(path));
	if (siz == -1)
		return -1;

	return __ldso_access(path, mode, exec_rpath, exec_runpath, rpath,
			     ld_library_path, runpath, deflib, flags_1, buf,
			     bufsiz);
}
...
hidden ssize_t __fdl_access(int fd, int mode, char *buf, size_t bufsiz)
{
	...
	/* Look for the shared object */
	return __fldso_access(fd, mode, exec_rpath, exec_runpath, NULL,
...
ssize_t __dl_access(const char *path, int mode, char *buf, size_t bufsiz)
{
	...
	ret = __fdl_access(fd, mode, buf, bufsiz);
